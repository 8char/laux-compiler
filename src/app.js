...
function compileFile(root, file, out) {
  ...
  bluebird.join(
    ...
    */
    ...
    const result = new CodeGenerator(compiled.code, compiled.compiledAST)
      .generate();
...
function compileFolder(root) {
  ...
  walker.on('file', (rootPath, fileStats, next) => {
    ...
    if (ext == '.laux') {
      const p = path.join(rootPath, fileStats.name);
...
function handleFileEvent(event, root, file, out) {
  ...
  if (fileExtension == 'laux') {
    ...
    const inPathNoExt = inPath.slice(0, -path.extname(inPath).length);
...
function watchFolder(root, out, workspace) {
  const watcher = chokidar.watch(path.join(root, '**/*.{lua,laux}'));
  ...
  watcher.on('unlink', (filePath) => {
    const relativePath = path.relative(root, filePath);
    const compiledPath = path.join(out, relativePath);
    const compiledPathNoExt = compiledPath.slice(0, -path.extname(compiledPath).length);
